...
- [第 3 章 Attention 的并行结构与优化路线](#第-3-章-attention-的并行结构与优化路线)
...
- [第 6 章 V2：Online Softmax——拆掉 softmax 的全局依赖](#第-6-章-v2online-softmax拆掉-softmax-的全局依赖)
...
- **为什么除以 $\sqrt{d}$**：若 $q$、$k$ 的各分量近似独立、方差为 1，则点积 $q \cdot k$ 的方差为 $d$。除以 $\sqrt{d}$ 把分数的方差归一回 1，避免 softmax 输入过大而饱和（也能缓解 2.5 节讨论的 exp 数值上溢，但不能根除）。
...
![Attention 三步计算与各级数据规模示意](./source/img/image.png)
...
三笔账合起来看：**算术强度（Arithmetic Intensity, AI）** 只有 $4.3\ \mathrm{GFLOP} / 268\ \mathrm{MB} \approx 16\ \mathrm{FLOP/Byte}$，低于典型 GPU 约 35 FLOP/Byte 的平衡点（2.6 节会解释这个平衡点的来历）。也就是说，**朴素 Attention 是 memory-bound 的**——尽管它内部有两个大矩阵乘，瓶颈却不在计算，而在 $N^2$ 中间矩阵反复过显存。
...
本章从零介绍理解全文所必需的概念。已熟悉 CUDA 的读者可以只读后四节——归约、数值稳定性、算术强度与流量分析，它们是 Attention 特有的工具；其余可以跳过。
...
softmax 的行内求 max、求 sum 都是**归约（reduction）**：把一组数缩成一个数（求和、求最大值、求最小值都属此类）。

**为什么归约在 GPU 上需要专门设计？** 串行写法 `for (j) res = fmaxf(res, x[j])` 每一步都依赖上一步的结果，整条链毫无并行可言——一行 4096 个元素就要串行 4096 步。反过来，如果让很多线程分头处理，每个线程只得到自己那部分的"部分结果"，最后仍要把这些部分结果**合并**成一个——合并本身怎么高效做，才是问题所在。GPU 的标准解法是**树形归约**：每一轮把参与的数两两配对合并，数量逐轮减半，$n$ 个数只需 $\log_2 n$ 轮（4096 个数 12 轮）。剩下的问题只有一个：**配对的两个线程之间，怎么读到对方手里的值？** 这取决于两个线程离得多"近"，恰好对应 2.1 节的线程组织层级：

| 两个线程的关系 | 通信手段 | 延迟 |
|------|---------|------|
| 同一 Warp 内（32 线程） | shuffle 指令：直接读对方的**寄存器** | 1～2 周期 |
| 同一 Block 内、不同 Warp | **共享内存**中转 + `__syncthreads()` 同步 | 20～30 周期 + 同步开销 |

于是高效实现分两级：**能在 Warp 内用寄存器解决的都在 Warp 内解决，只有跨 Warp 的最后一步走共享内存**。

**第一级：Warp 内归约（寄存器直传）。** 先认识核心工具 `__shfl_xor_sync(mask, v, offset)`——Warp 内的"寄存器交换"指令：

- 每个线程在 Warp 内的编号称为 **lane**（0～31，即 `threadIdx.x % 32`）；
- 该指令返回**「lane 号 = 自己的 lane 号 XOR offset」的那个线程手里的 `v` 值**。例如 offset = 16 时，lane 0 拿到 lane 16 的 `v`，lane 16 也同时拿到 lane 0 的 `v`——相距 offset 的两个 lane 一条指令互看对方的寄存器，不经过任何内存；
- `mask = 0xffffffff`（32 个 bit 全 1）表示全部 32 个 lane 都参与。

用它实现树形归约，offset 依次取 16、8、4、2、1，因配对连线呈蝶形交叉，称**蝶形（butterfly）模式**：

```cuda
// Warp 内求最大值：调用时每个 lane 各持一个 v（共 32 个数），
// 5 步之后，每个 lane 的 v 都变成这 32 个数的最大值
__device__ float warpReduceMax(float v) {
    for (int offset = 16; offset > 0; offset >>= 1)
        // 取来 lane^offset 号伙伴的 v，与自己的 v 取大：
        //   offset=16：0↔16, 1↔17, ...  每个 lane 持有"2 个数的 max"
        //   offset= 8：0↔8,  1↔9,  ...  持有"4 个数的 max"
        //   ...每步覆盖范围翻倍，5 步后覆盖全部 32 个数
        v = fmaxf(v, __shfl_xor_sync(0xffffffff, v, offset));
    return v;   // 32 个 lane 都得到全 Warp 最大值
}

// 求和版本：结构完全相同，合并操作由 fmaxf 换成 +
...
用 4 个 lane 的迷你例子把 `warpReduceMax` 走一遍（4 个数只需 offset = 2、1 两步；真实的 32 lane、5 步原理完全相同）：

| | lane 0 | lane 1 | lane 2 | lane 3 |
|--|--|--|--|--|
| 初始 $v$ | 3 | 7 | 5 | 1 |
| offset=2：与 lane$\oplus$2 互看，取大 | max(3,5)=**5** | max(7,1)=**7** | max(5,3)=**5** | max(1,7)=**7** |
| offset=1：与 lane$\oplus$1 互看，取大 | max(5,7)=**7** | max(7,5)=**7** | max(5,7)=**7** | max(7,5)=**7** |

两步后 4 个 lane 全部持有最大值 7。注意 XOR 配对的妙处在于**对称**：每一步所有 lane 都在干活、都在积累部分结果，结束时**人人持有最终答案**。这一点对 softmax 特别有用——归约出的 $m$ 和 $l$ 是每个线程后续都要用的（人人都要算 $e^{x_j - m}$），对称模式直接省掉了"一个线程算完、再广播给其他人"的额外步骤。

**第二级：Block 内归约。** shuffle 只能在 Warp 内部使用；一个 Block（例如 256 线程 = 8 个 Warp）的归约必须借共享内存中转，分四步：

```cuda
// Block 内求最大值：调用时每个线程各持一个 v（共 blockDim.x 个数），
// 返回时每个线程都得到全 Block 的最大值
__device__ float blockReduceMax(float v) {
    __shared__ float warpRes[32];   // 存各 Warp 的部分结果；Block 至多 1024 线程 = 32 个 Warp，够用
    int lane = threadIdx.x % 32;    // 本线程在 Warp 内的编号（0~31）
    int wid  = threadIdx.x / 32;    // 本线程所属的 Warp 编号

    // 步骤 1：各 Warp 内部先归约（纯寄存器，最快的部分干掉绝大多数工作）
    //         结束后每个 Warp 的 32 个 lane 都持有本 Warp 的最大值
    v = warpReduceMax(v);

    // 步骤 2：每个 Warp 派 lane 0 当代表，把本 Warp 的部分结果写进共享内存
    if (lane == 0) warpRes[wid] = v;
    __syncthreads();                // 跨 Warp 读写共享内存，必须先同步：等所有代表写完

    // 步骤 3：部分结果至多 32 个，恰好一个 Warp 就能装下——
    //         让 Warp 0 的 lane i 认领第 i 个部分结果，再做一次 Warp 归约
    int nWarp = (blockDim.x + 31) / 32;                // 实际的 Warp 数（256 线程 → 8）
    v = (lane < nWarp) ? warpRes[lane] : -INFINITY;    // 没有对应部分结果的 lane 用 -inf 占位
                                                       //（-inf 是 max 的"单位元"，不影响结果）
    if (wid == 0) v = warpReduceMax(v);                // 只有 Warp 0 执行这次归约

    // 步骤 4：此刻只有 Warp 0 持有最终结果，经共享内存广播给全 Block
    __shared__ float result;
    if (threadIdx.x == 0) result = v;
    __syncthreads();                // 等写入完成，所有线程才能读
    return result;
}
```

`blockReduceSum` 同理，只需两处替换：`fmaxf` 换成 `+`，占位值 `-INFINITY` 换成 `0`（加法的单位元）。

这对组件将在 V1 的融合 softmax（5.2 节）中直接使用，配合方式是"先线程内、再 Block 内"：一行 4096 个元素分给 256 个线程，每个线程先串行扫自己名下的 16 个元素得到线程级部分结果，再调用 blockReduce 把 256 个部分结果合并成全行的 max 或 sum——两级归约负责的正是后一半。
...
"三遍扫描"意味着三倍读流量——这个数字在 2.7 节的流量账本和 V2（第 6 章）的优化中会反复出现。

> **一个值得提前建立的观念**：pass 3 的 $y_j$ 是 softmax 作为独立算子的最终输出；但放回 Attention 中，它只是喂给下一步 $O = PV$ 的中间量——真正需要的是加权和 $\sum_j y_j v_j$。注意到 $y_j = e^{x_j - m}/l$ 中所有项共用同一个分母 $l$，于是可以先用未归一化的分子 $e^{x_j - m}$ 与 $v_j$ 完成加权累加，最后再统一除以 $l$——$y_j$ 全程无需显式算出。第 6 章的 Online Softmax 正是沿着这个思路，把整个 $P$ 矩阵从计算中"消掉"。
...
以第 1 章的朴素实现为例：$S$ 被写 1 遍，safe softmax 的三遍扫描把它读 3 遍，$P$ 被写 1 遍、又被读 1 遍——合计 **6 遍**、约 384 MB，而有效输入输出总共只有 4 MB。后文每个版本都会用这个口径记账，3.3 节有整条优化路线的流量预览表。

---

## 第 3 章 Attention 的并行结构与优化路线

预备知识备齐，回到 Attention 本身。动手写 kernel 之前还差两步：先分析清楚**哪些计算互不依赖、可以同时算**——这决定了后文每个版本的线程组织；再把整条优化路线一次看完，带着地图上路。
...
- 只读共享反而是好事：一份 K/V 搬到片上可以喂给很多行——这正是数据复用的机会（第 7 章的 FlashAttention 把 K/V tile 装进共享内存、供块内所有行共用，用的就是这个性质）。
...
| 特殊场景 | decode 阶段并行度骤减 | Flash-Decoding（第 9 章） |
...
用 2.7 节"数 $N^2$ 流量过 HBM 几遍"的口径（一遍 = 64 MB，默认参数），整条路线的效果可以先在纸面上预估：
...
后文每个版本的效果，都可以先用这张表预估，再看实现细节。下面从基准实现开始。
...
|------|------|
| kernel 1 写 S | 64 MB |
| kernel 2 读 S 三遍（safe softmax 的三遍扫描，见 2.5 节） | 192 MB |
| kernel 2 写 P | 64 MB |
| kernel 3 读 P | 64 MB |
...
1. **每线程独自处理一整行**：行内 4096 个元素串行扫描；且相邻线程（处理相邻行）同一时刻访问的地址相距一整行，完全无法合并访存（违反 2.2 节规则）；
2. **三遍扫描，三倍读流量**。
...
要消灭它们，就必须把三步彻底融成**一个** kernel：$S$ 的每个分块算出来就地喂给 softmax，softmax 的结果就地喂给 PV 累加。拦路的正是 softmax 的**全局依赖**——分母要等整行算完才知道。第 6 章拆掉这把锁。

---

## 第 6 章 V2：Online Softmax——拆掉 softmax 的全局依赖

本章是全文的数学核心。V2 做的事情用一句话概括：**把 softmax 从"必须先看完全行才能动手"改写成"来一段算一段、错了可以事后修正"的流式递推**。它单独使用收益有限，但它是 V3 分块融合的前提。

推导按四步递进：逐元素递推 → 分块合并 → 连输出一起递推 → 从单行推广到多行。每一步都只在前一步上加一个很小的想法。
...
注意此时**绝不能**直接取 $l_1 + l_2 + l_3 \approx 3.2034$ 当全局分母。按定义，全局分母应该以**全局**最大值 $m = \max(m_1, m_2, m_3) = 5$ 为基准，把 6 个元素全部代入算出来是：

$$
\begin{aligned}
l = \sum_{j=1}^{6} e^{x_j - 5} &= e^{1-5} + e^{3-5} + e^{2-5} + e^{5-5} + e^{4-5} + e^{0-5} \\
&= 0.0183 + 0.1353 + 0.0498 + 1 + 0.3679 + 0.0067 \\
&\approx 1.5781
\end{aligned}
$$

直接相加的 3.2034 与正确答案 1.5781 相差一倍多。错在哪里？$l_1$ 中每一项以 $m_1 = 3$ 为基准、$l_2$ 以 $m_2 = 5$ 为基准、$l_3$ 以 $m_3 = 4$ 为基准——三块的"参考系"各不相同，直接相加没有意义。

怎么把一个以 $m_1$ 为基准的部分和，换算到基准 $m$ 上？还是 6.2 节用过的那个指数恒等式（**换基准**）。对块 1 中的任意一项：
...
与前面按定义算出的 1.5781 完全一致——换基准后再相加，得到的就是正确的全局分母。

不过，一次性合并有个前提：要先知道全局最大值 $m$，这意味着所有块都算完才能动手。实际计算中块是**依次到来**的（FlashAttention 里 K/V 块沿序列一块块加载），我们希望**来一块就并一块**。办法与 6.2 节的逐元素递推完全相同：维护全局状态 $(m, l)$——"目前为止见过的最大值"与"以它为基准的累积分母"，初始 $m = -\infty$、$l = 0$；每接入一个块，先把基准更新为两者中的较大者，再把"旧累积"和"新块"**各自**换到新基准后相加：
...
对照一下：这就是 6.2 节的递推式，只是"新元素 $x_{t+1}$"换成了"新块的局部统计量 $(m_b, l_b)$"（单个元素相当于 $m_b = x_{t+1}$、$l_b = 1$ 的块）。
...
初始的空状态被 $e^{-\infty} = 0$ 自然清零，无需特判——7.3 节的 FlashAttention 代码把 $m$ 初始化为 `-INFINITY`，正是为了让第一块走同一条公式。当前状态 $(m, l) = (3,\ 1.1353)$。
...
**验证**：三次递推的最终状态 $l \approx 1.5781$，与本节开头按定义直接算出的全局分母 $l = \sum_{j=1}^{6} e^{x_j - 5} \approx 1.5781$ 一致（文中数值保留 4 位小数，末位或有 ±1 的舍入差；用全精度计算则**严格逐位相等**）——分块合并是精确变换，不是近似。

三点观察：

- 6.2 节的逐元素递推就是"块大小 = 1"的特例——新元素自成一块（$m_b = x_{t+1}$、$l_b = 1$），块粒度递推随之退化为 6.2 节的递推式；反过来说，上面三次"接入"就是 6.2 节递推在块粒度上的重演；
- 合并是**可结合的**：上例改成先合并块 2 与块 3、再并入块 1，结果不变；多块可以两两归并、任意分组，甚至树状并行归并——9.3 节 Flash-Decoding 的跨 Block 归并直接建立在这个性质上；
- 块内计算（求 $m_b$、$l_b$）是规整的批量运算，GPU 可以用矩阵/向量指令高效完成；块间合并只有标量级开销——这正是"分块"对硬件友好的原因。

### 6.4 更进一步：连输出的加权和也能在线递推

单独优化 softmax 只省一遍扫描，真正的威力在于把第③步（$O = PV$）也拉进递推。仍然只看 $S$ 的一行：记该行的分数为 $x_j$（与 6.3 节同），每个分数对应一个 value 向量 $v_j$——即 $V$ 矩阵的第 $j$ 行，长度为 head 维 $d$。这一行的输出是概率加权和：

$$
o = \sum_j \mathrm{softmax}(x)_j \, v_j = \frac{\sum_j e^{x_j - m} \, v_j}{l}
$$

分母 $l$ 的流式递推已经被 6.2/6.3 节解决；再看分子——它同样是"以 $m$ 为基准的指数加权和"，与 $l$ 的唯一区别是每项多乘了一个 $v_j$，因此**基准变化时服从与 $l$ 完全相同的修正规律**（整体乘 $e^{\,m_{\mathrm{old}} - m_{\mathrm{new}}}$）。于是维护第三个量（长度为 head 维 $d$ 的向量），即**未归一化输出**：

$$
\tilde{o} = \sum_j e^{x_j - m} \, v_j
$$

照搬 6.3 节的分块流程：每块在 $(m_b, l_b)$ 之外，再多算一个局部分子

$$
\tilde{o}_b = \sum_{x_j \in \text{块 } b} e^{x_j - m_b} \, v_j
$$

块粒度递推只需在 6.3 节的公式上**新增一行**——$\tilde{o}$ 的更新与 $l$ 逐字相同，用的是同一个补偿系数：
...
全部块接入后收尾：$o = \tilde{o} / l$，**全程唯一的一次除法**。（块大小取 1 即逐元素版本：$\tilde{o} \leftarrow \tilde{o} \cdot e^{\,m - m_{\mathrm{new}}} + e^{\,x_{t+1} - m_{\mathrm{new}}} \, v_{t+1}$，与 6.2 节的 $l$ 递推式一一对应。）

**把 6.3 节的数值实例接着算完。** 沿用分数 $x = [\,1,\ 3 \mid 2,\ 5 \mid 4,\ 0\,]$，为每个分数配一个 value（取 $d = 1$，$v_j$ 为标量，便于手算）：

$$
v = [\,10,\ 20 \mid 30,\ 40 \mid 50,\ 60\,]
$$

各块的局部分子（公式 → 代值，与 6.3 节算 $l_b$ 时并行完成）：
...
流式接入三块。$m$、$l$ 的更新与 6.3 节完全相同（$m: -\infty \to 3 \to 5 \to 5$，$l: 0 \to 1.1353 \to 1.2034 \to 1.5781$），此处只展开新增的 $\tilde{o}$ 一行——注意每步乘的补偿系数与 6.3 节中 $l$ 用的**一模一样**：
...
$$
o = \frac{\tilde{o}}{l} = \frac{63.1817}{1.5781} \approx 40.04
$$

再与定义式对拍验证：直接以全局最大值 $m = 5$ 为基准，把 6 对 $(x_j,\ v_j)$ 逐项代入分子，分母即 6.3 节按定义算出的 $l \approx 1.5781$：

$$
\begin{aligned}
o_{\text{定义}} = \frac{\sum_{j=1}^{6} e^{x_j - 5} \, v_j}{l}
&= \frac{e^{1-5} \cdot 10 + e^{3-5} \cdot 20 + e^{2-5} \cdot 30 + e^{5-5} \cdot 40 + e^{4-5} \cdot 50 + e^{0-5} \cdot 60}{1.5781} \\
&= \frac{0.1832 + 2.7067 + 1.4936 + 40 + 18.3940 + 0.4043}{1.5781} \\
&= \frac{63.1818}{1.5781} \approx 40.04
\end{aligned}
$$

定义式的分子 63.1818 与流式递推的 63.1817 仅差末位舍入（4 位小数的累积误差；全精度下严格相等），输出同为 40.04——$\tilde{o}$ 与 $l$ 用同一套补偿系数、各自独立累加，6.3 节的精确性原样继承到了输出上。

顺带把例子里被简化掉的一层结构说清楚：为了手算，上例取了 $d = 1$，$v_j$ 和 $\tilde{o}$ 都退化成了标量。一般情形下 $v_j$ 是 $V$ 矩阵的第 $j$ **行**（长度为 head 维 $d$ 的行向量），$\tilde{o}$ 是 $d$ 维累加器。此时递推式不需要任何改动——$\tilde{o} \cdot e^{\,m - m_{\mathrm{new}}}$ 与 $\tilde{o}_b \cdot e^{\,m_b - m_{\mathrm{new}}}$ 都是"向量乘标量"，$d$ 个分量乘的是**同一个**补偿系数。换句话说，$d > 1$ 就是 $d$ 条并行的标量递推（每个分量一条，与上例完全相同），共享同一套 $(m, l)$：max 和分母只由分数 $x_j$ 决定，与 value 的维度无关。这正是 6.5 节表格中"单行时 $\tilde{o}$ 是 $d$ 维向量"的含义，也是 7.3 节代码里数组 `o[D]` 只配一对标量 `m`、`l` 的原因。
...
前三节已经在**一行**分数上建好了完整的流式递推：状态 $(m, l, \tilde{o})$，来一块更新一次，收尾一除。推广到 Attention 的真实形态（$N$ 行同时算）只需一步：softmax 是**按行独立**的（3.1 节的并行分析），所以**每一行都有自己的一套 $(m, l, \tilde{o})$**，行与行互不干扰。把 $Q$ 按行切成块（每块 $B_r$ 行）后，统计量从标量升级为向量：
...
对应地，$K$/$V$ 沿序列切成列块（每块 $B_c$ 行）——6.3/6.4 节一维递推里"接入一个块"，在这里就是"接入一个 K/V 列块"。一次接入处理的分数是 $B_r \times B_c$ 的小矩阵 $S_{\mathrm{blk}} = Q_{\mathrm{blk}} K_{\mathrm{blk}}^\top$（已含缩放），更新流程**逐行并行**（$\odot$ 为逐元素乘，$r$ 沿行广播）：
...
区别只在计算时机：数值实例的设定里各块**先并行算好**局部量（基准只能取本块的 $m_b$），接入时不得不补一次换基准；而在 FlashAttention 的循环里，接入某块时才现算它的指数，此时 $m_{\mathrm{new}}$ 已知，**一步到位直接以 $m_{\mathrm{new}}$ 为基准**更省——新块一侧的换基准乘法就地消失，只有算完的旧累积仍需乘 $r$。7.2 节的算法与 7.3 节的代码用的正是这个形式（$\tilde{P} = e^{\,S_{\mathrm{blk}} - m_{\mathrm{new}}}$）。
...
至此，FlashAttention 的全部数学已经就位；剩下的是把这套"逐行向量递推"映射到 GPU 的线程组织上——7.3 节的实现中，每行一套的 $m$、$l$、$\tilde{o}$ 恰好按"每线程一行"放进各线程的寄存器。
...
把 6.5 节的逐行分块递推原样搬上 GPU，并让所有中间量驻留片上。整体是两层循环：
...
对照 2.7 节的流量账本：$S$ 和 $P$ 的 HBM 读写**归零**；HBM 流量只剩 $Q$ 读 1 遍、$K$/$V$ 各被每个 Q 块读 1 遍、$O$ 写 1 遍。显存占用从 $O(N^2)$ 降到 $O(N)$（只需给每行留 $m$/$l$ 统计量，甚至可以不留）。
...
2. **对每个 K/V 块 $j = 0, \dots, T_c - 1$（内层顺序循环）：**
   ...
   5. $r = e^{\,m - m_{\mathrm{new}}}$ —— 逐行补偿系数（6.5 节的 $r$）；
...
步骤 2.3～2.8 就是 6.5 节逐行更新公式的原文照录，正确性完全由那套递推与合并公式保证——FlashAttention 是**精确算法**，不是近似。
...
// Q/K/V/O: [BH, N, D]（BH = batch*heads）
// 要求：N 是 Bc 的倍数；D ≤ 模板上限；Bc 是 Br 的倍数（保证 causal 整块跳出时全 Block 同步，
//       否则部分线程先 break、跳过 __syncthreads() 会造成死锁）
...
__global__ void flash_attn_v3(const float* Q, const float* K,
    ...
    for (int j0 = 0; j0 < N; j0 += Bc) {
        if (causal && j0 > qRow) break;   // 整块都在掩码之外，直接结束（见 7.5 节）
        ...
        for (int c = 0; c < Bc; c++) {
            ...
            l = l * corr + p;
            for (int x = 0; x < D; x++)
                o[x] = o[x] * corr + p * Vs[c][x];   // o~ 递推（6.4 节的输出递推）
            m = m_new;
        }
...
结构上，它与 GEMM 优化中经典的共享内存分块（Block Tiling）完全同构：**K/V tile 是"料盒"（共享内存，滑动覆盖），q/õ/m/l 是"越攒越多的私有状态"（寄存器）**，两次 `__syncthreads()` 的语义也一样（没到齐不许吃 / 没吃完不许撤）。

> 教学版为清晰做了取舍：块内分数逐个（`c` 循环）处理——即"块大小 = 1"的逐元素递推，$S_{ij}$ 未显式成块；点积 `q·k_c` 未做 Warp 级并行；未用 Tensor Core。生产实现（flash-attn 库 / CUTLASS）把 ①② 换成分块矩阵乘 + Tensor Core，并按块粒度公式整块更新，骨架不变。

### 7.4 流量与显存账本

按 2.7 节流量账本的口径（$N = 4096$、$d = 64$、fp32、$B_r = 64$）：
...
V3 完成了全文的主线任务：$N^2$ 流量归零、显存 $O(N)$、三步融合为一个 kernel。但按 3.2 节"每个版本解决上一版暴露的瓶颈"的节奏，本章也留下了三个问题，正好构成第 8 章的三条线索：
...
V3 已经解决了访存的主要矛盾——$N^2$ 流量归零之后，算子回到 compute-bound，瓶颈转移到"片上计算本身是否高效"。本章顺着 7.6 节的三条线索展开：先把历史上的 FA-1 原始算法完整摆出来当**靶子**（8.1），然后逐条拆解 FA-2（2023）的四项改进——交换循环（8.2）、split-Q（8.3）、延迟归一化（8.4）、负载均衡（8.5），最后汇总成 FA-2 完整算法（8.6）、落成教学版 kernel（8.7），并给出性能定位（8.8）。

### 8.1 靶子：FA-1 的原始算法

7.1 节的历史注说过：FlashAttention-1 的原始实现与第 7 章的自然形式是**反的**。下面是它的完整算法（FlashAttention-1 论文 Algorithm 1 的忠实转述），记号与 7.2 节相同（$T_r = N/B_r$、$T_c = N/B_c$）。请特别留意加粗的三处——它们就是 FA-2 四项改进中前三项的靶心：
...
数学上它与 7.2 节的算法**逐位等价**（都是第 6 章递推的合法组织方式，10.3 节的 PyTorch 模拟可直接验证这一点），但工程上处处别扭：
...
前两条改进解决"怎么分工"，这一条解决"每个人干的活里有没有废动作"。Tensor Core 时代矩阵乘吞吐极高，夹在中间的**逐元素运算**（exp、补偿乘法、除法）反而成为新短板。对照 FA-1 算法的步骤 2.2.5——它维护已归一化的 $O$，每个内层步都要"乘回 $l_i$、除以 $l_{\mathrm{new}}$"。为什么这（直觉上更符合 softmax 输出概率的定义）反而是坏主意？6.4 节的合并推导已给出数学层面的答案——归一化在合并时被原样抵消，这里补上硬件层面的两条：
...
- **归一化推迟到最后**：内层循环只维护未归一化的 $\tilde{o}$（7.3 节的写法本就如此，`inv_l` 只在收尾出现一次）；
...
四条改进汇总之后，FA-2 的循环结构就是 7.2 节——第 7 章本就按这个顺序讲解，此处不再重复，只补充三点工程增量：
...
7.3 节的教学版已经采用了 FA-2 的外层 Q 循环（改进一），但 Block 内部是"**每线程一行**"——点积和 $\tilde{o}$ 累加都由单个线程串行完成，这正是它自己声明的缺陷。下面的升级版把改进二的 split-Q 落到 Warp 粒度：**一个 Warp 负责一行**，行内的 head 维 $d$ 切给 32 个 lane 并行。与 7.3 节的差异一览：
...
__global__ void flash_attn_v4(const float* Q, const float* K,
    ...
    for (int j0 = 0; j0 < N; j0 += Bc) {
        ...
        for (int c = 0; c < Bc; c++) {
            ...
            // 在线递推与 7.3 节完全相同（m、l 在各 lane 上冗余但数值一致）
            float m_new = fmaxf(m, s);
...
- **$q$/$\tilde{o}$ 的 lane 切分**（索引 `x * 32 + lane`）：同一时刻 Warp 内 32 个 lane 访问的地址连续——装载 $q$、读 K/V tile、写回 $O$ 全部满足合并访存（2.2 节规则），这是 7.3 节"每线程一行"版本做不到的（它读 `Ks[c][x]` 时整个 Warp 广播同一地址，写 $O$ 时各线程相距一整行）；
- **蝶形归约后每个 lane 都持有完整的 $s$**（2.4 节的性质），于是 $m$、$l$ 的递推在 32 个 lane 上冗余进行、数值逐位一致，省去一次广播——"本行的一切状态都在本 Warp 内部"，正是 split-Q 的"行独立走完全流程"；
- **代价与取舍**：每 Warp 只管一行使 $B_r$ 缩到 8，K/V 全量读取次数 $N/B_r$ 变大（7.4 节的账本变差 8 倍）。生产实现让每个 Warp 处理多行（如 16 行）、$B_r$ 保持 64~128，兼得复用与并行——那属于寄存器分块的范畴，骨架不变。
...
据 FlashAttention-2 论文报告，在 A100/H100 上其 forward 可达理论算力的 50%～70%（fp16/bf16，Tensor Core），与高度调优的 GEMM 同一量级——考虑到它还内嵌了 softmax 与掩码逻辑，这已接近该算法结构的硬件上限。后续的 FlashAttention-3 进一步利用 Hopper 的 TMA 与 wgmma 异步流水，属于"双缓冲 → 硬件异步化"这条思想脉络的延伸。

---

## 第 9 章 场景扩展：推理 Decode 阶段与 Flash-Decoding

前八章优化的都是"$N$ 行 query 一起算"的 prefill 形态。推理的 decode 阶段形态完全不同，2.6 节"先判形态、再选武器"的方法论需要重新套用一遍：**先判断算子形态（compute-bound 还是 memory-bound），再选武器**。
...
归并公式就是 6.3 节的块粒度递推做一步——把两份部分结果分别当作"旧累积"与"新块"合并即可。值得注意的是，这里各段**先并行算好**局部量 $(m^{(s)}, l^{(s)}, \tilde{o}^{(s)})$、归并时再换基准，是分块合并最原始的形式；FlashAttention 主循环那种"直接以 $m_{\mathrm{new}}$ 为基准"的省乘法写法在这里用不了——各段在不同 Block 上并行计算，算的时候彼此不知道对方的最大值：
...
段数多于 2 时两两归并即可——归并的**可结合性**（6.3 节的数值实例验证过）保证任意分组、任意顺序的结果都相同，甚至可以树状并行归并。这与 GEMM 中的 Split-K（K 维切给多 Block 再归约）是同一族技巧：**当天然并行维度不足时，牺牲一次归并换取并行度**。
...
3. 写出 $\left(m^{(s)},\ l^{(s)},\ \tilde{o}^{(s)}\right)$ 到工作区——**不做归一化**（6.4 节的结论：归一化了归并时还得乘回来）。

**Kernel 2：归并**——Grid = $\mathrm{batch} \times \mathrm{heads}$，每个 Block 归并一份：

1. 初始化 $m = -\infty$、$l = 0$、$\tilde{o} = 0$；
2. **对 $s = 0, \dots, S-1$：** 用 9.3 节的归并公式两两合并——把 $\left(m^{(s)}, l^{(s)}, \tilde{o}^{(s)}\right)$ 当作"新块"，$(m, l, \tilde{o})$ 当作"旧累积"；
3. $o = \tilde{o} / l$，写回。
...
沿用 8.7 节 FA-2 教学版的 Warp 组织——一个 Warp 干一份活，lane 沿 head 维切分、蝶形归约拼点积——两个 kernel 的代码几乎就是它内层循环的"拆开重排"：
...
- **两种基准形式的分工**在代码里看得很清楚：Kernel 1 内部"边扫边以当前 $m_{\mathrm{new}}$ 为基准"（顺序计算时省一次换基准乘法），Kernel 1 与 Kernel 2 之间则"先算好局部量、归并时再换基准"（并行计算时彼此不知道对方的最大值）——9.3 节的理论讨论在此落到了实处。
...
原理讲完了，本章回答四个实践问题：日常怎么用现成实现、手写的 kernel 怎么验证对不对、动手写 CUDA 之前怎么先把算法调对，以及什么情况下才值得手写。
...
from torch.nn.attention import sdpa_kernel, SDPBackend    # 需要 PyTorch >= 2.3

B, H, N, D = 1, 1, 4096, 128       # batch / heads / 序列长度 / head 维度

q = torch.randn(B, H, N, D, device='cuda', dtype=torch.float16)
k, v = torch.randn_like(q), torch.randn_like(q)

# 用法一：自动选后端（优先 FlashAttention，条件不满足时自动回退）
o = F.scaled_dot_product_attention(q, k, v, is_causal=True)

# 用法二：强制指定后端（此处强制 FlashAttention；硬件或数据类型不支持时会直接报错）
with sdpa_kernel(SDPBackend.FLASH_ATTENTION):
    o = F.scaled_dot_product_attention(q, k, v, is_causal=True)

print(o.shape, o.dtype)            # torch.Size([1, 1, 4096, 128]) torch.float16
```
...
对拍的框架如下——为了让代码可以直接运行，"被测实现"先用 PyTorch SDPA 占位，验证自己的 kernel 时替换成扩展调用（如 7.3 节教学版封装出的 `my_ext.flash_attn`）即可：
...
def reference(q, k, v, causal=True):
    """朴素参考实现（V0 逻辑）：物化 N×N 矩阵，慢但直观、不易错，适合当裁判"""
    s = q @ k.transpose(-1, -2) / math.sqrt(q.shape[-1])        # ① S = QKᵀ/√d
    if causal:
        n = q.shape[-2]
        mask = torch.triu(torch.ones(n, n, device=q.device, dtype=torch.bool),
                          diagonal=1)                           # 上三角（j > i）为 True
        s = s.masked_fill(mask, float('-inf'))                  # 置 -inf，softmax 后即为 0
    return torch.softmax(s, dim=-1) @ v                         # ② P = softmax(S)  ③ O = PV

torch.manual_seed(0)                                            # 固定种子，问题可复现
B, H, N, D = 2, 4, 1024, 64
q = torch.randn(B, H, N, D, device='cuda')
k, v = torch.randn_like(q), torch.randn_like(q)

# 被测实现：替换成自己的扩展，如 o_mine = my_ext.flash_attn(q, k, v, causal=True)
o_mine = F.scaled_dot_product_attention(q, k, v, is_causal=True)
o_ref  = reference(q, k, v, causal=True)

print(torch.allclose(o_mine, o_ref, rtol=1e-3, atol=1e-3))     # True 即通过
print((o_mine - o_ref).abs().max().item())                     # fp32 下应为 ~1e-6 量级
```
...
- **测极端分布**：给一行注入一个特别大的分数（如 +50），验证 safe/online softmax 没有 NaN——这正是 2.5 节 exp 上溢问题的回归测试；
...
def fa1_outer_kv(Q, K, V):
    """FA-1 风格：外层 KV，内层 Q —— (o~, m, l) 存于全局张量，反复读改写"""
    O = torch.zeros(N, D, device=dev)                    # 未归一化输出 o~（收尾统一除以 l）
    ...
    for j in range(Tc):                                  # 外层：KV 块
        ...
        for i in range(Tr):                              # 内层：Q 块
            ...
            O_i, m_i, l_i = O[sl], m[sl], l[sl]          # 灾难点：取回历史进度（对应 8.1 节步骤 2.2.1 的 HBM 读）
            ...
            O[sl] = O_i * corr + P @ V_j                 # 灾难点：写回全局进度（对应步骤 2.2.6 的 HBM 写）
            m[sl] = m_new
    return O / l                                         # 延迟归一化：除法只在最后做一次

def fa2_outer_q(Q, K, V):
    """FA-2 风格：外层 Q，内层 KV —— (o~, m, l) 是局部变量，收尾只写一次"""
    ...
    for i in range(Tr):                                  # 外层：Q 块
        ...
        for j in range(Tc):                              # 内层：KV 块（与 FA-1 的更新公式逐行相同）
            K_j, V_j = K[j*Bc:(j+1)*Bc], V[j*Bc:(j+1)*Bc]
            S = Q_i @ K_j.T * scale
...
print((out1 - out2).abs().max().item())   # 0.0：两种顺序逐位一致（合并公式精确）
print((out2 - ref ).abs().max().item())   # ~1e-6 量级：与参考实现的浮点求和顺序差异

# ---- 计时：CUDA event + 预热（正是 10.2 节的计时建议）----
def bench(fn, iters=5):
    for _ in range(2):                    # 预热：排除首次运行的内存分配等一次性开销
        fn(Q, K, V)
    ...
    torch.cuda.synchronize()              # 等 GPU 全部执行完毕才能读取计时
    return beg.elapsed_time(end) / iters  # 单位：毫秒

print(f"FA-1 风格（外层 KV）: {bench(fa1_outer_kv):.2f} ms")
print(f"FA-2 风格（外层 Q） : {bench(fa2_outer_q):.2f} ms")
```

预期 FA-2 风格明显更快，且 $N$ 越大差距越大，机理如下：
...
| 学习原理 | 手写 V0~V4（本文，7.3 与 8.7 节均有教学版实现） |
...
| 理解 FlashAttention | 精读第 6 章（数学）→ 第 7 章（实现），手推一遍 6.2 的递推与 6.3 的分块合并 |
