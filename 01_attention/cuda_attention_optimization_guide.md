...
6.2 的递推是**逐元素**的。把它"批量化"就得到一个等价且对 GPU 更友好的形式：**把数据切成块，每块先独立算出局部统计量，块间再合并**——块内是规整的并行运算，块间只需一步修正。这正是 FlashAttention 实际采用的组织方式，值得单独推一遍。

以 4 个元素、切成两块为例：

```
x = [x₁, x₂ | x₃, x₄]           Block 1 = [x₁, x₂]，Block 2 = [x₃, x₄]

Block 1（独立计算）: m₁ = max(x₁, x₂)     d₁ = exp(x₁−m₁) + exp(x₂−m₁)
Block 2（独立计算）: m₂ = max(x₃, x₄)     d₂ = exp(x₃−m₂) + exp(x₄−m₂)
```

现在要合并出全局结果。全局最大值显然是 m = max(m₁, m₂)；但全局分母**绝不能**直接取 d₁ + d₂——d₁ 中每一项以 m₁ 为基准、d₂ 以 m₂ 为基准，两者"参考系"不同，直接相加没有意义。

修复的办法还是 6.2 用过的那个指数恒等式（**换基准**）：

```
exp(x₁ − m) = exp(x₁ − m₁) · exp(m₁ − m)
              └─────┬─────┘   └────┬────┘
              d₁ 中的旧项      补偿系数（整块共用同一个）
```

给每块的部分和整体乘上各自的补偿系数，就统一到了同一基准：

```
m = max(m₁, m₂)
d = d₁·exp(m₁ − m) + d₂·exp(m₂ − m)
```

两种情形自查（与 6.2 一致）：若 m = m₁，则 Block 1 的补偿系数为 exp(0) = 1（不需修正），Block 2 被乘上 exp(m₂ − m) ≤ 1 缩小；反之亦然。合并结果与一次性算全量**逐位一致**，不是近似。

一个帮助记忆的比喻：分省统计全国的"指数加权和"。传统 safe softmax 相当于必须先跑遍全国找出首富（全局 max）才能开始算任何东西；分块合并则允许每个省先以**本省首富**为基准把部分和算好记在小本子上，两省合并时若发现对方首富更有钱，只需给本省的部分和按财富差额**打个折**（乘 exp(m₁ − m)），完全不必重算任何人。FlashAttention 里这个"小本子"就是片上的寄存器/共享内存。

三点观察：

- 6.2 的逐元素递推就是"块大小 = 1"的特例——新元素自成一块（m₂ = x_{t+1}、d₂ = 1），合并公式退化为 6.2 的递推式；
- 合并是**可结合的**：多块可以两两归并、任意分组，甚至树状并行归并——第 9 章 Flash-Decoding 的跨 Block 归并直接建立在这个性质上；
- 块内计算（求 m₁、d₁）是规整的批量运算，GPU 可以用矩阵/向量指令高效完成；块间合并只有标量级开销——这正是"分块"对硬件友好的原因。
...
从 6.3 的分块合并视角再推一遍，还能看出一个重要设计决策的来由。假设每块**先做完整的局部 softmax**，得到"局部归一化输出"（把该块当成全量时的正确答案）：

```
O₁ = ( exp(x₁−m₁)·v₁ + exp(x₂−m₁)·v₂ ) / d₁        ← 块内自己归一化过了
```

全局输出按定义展开、对 Block 1 部分提取补偿系数：

```
O = [ (exp(x₁−m)v₁ + exp(x₂−m)v₂) + (exp(x₃−m)v₃ + exp(x₄−m)v₄) ] / d
                                                     （m、d 为全局值）

分子的 Block 1 部分 = exp(m₁−m) · (exp(x₁−m₁)v₁ + exp(x₂−m₁)v₂)
                    = exp(m₁−m) · d₁·O₁              ← 括号里恰是 d₁·O₁

于是：  O = [ d₁·exp(m₁−m)·O₁ + d₂·exp(m₂−m)·O₂ ] / d
```

合并公式成立，但请注意合并时用的是 **d₁·O₁**——块内先除以 d₁，合并时又乘回来，归一化被原样抵消了。这透露出：**块内归一化是多余的**。若一开始就维护未归一化的分子 o~₁ = d₁·O₁（正是本节递推维护的量），合并式立刻简化：

```
o~ = o~₁·exp(m₁−m) + o~₂·exp(m₂−m)        —— 与分母 d 的合并规律完全相同
O  = o~ / d                                —— 除法只做最后一次
```

分子 o~ 与分母 d 在整个过程中**各自独立**地用同一补偿系数累加、互不干扰，除法从"每块一次"推迟为"全程一次"。这个"分子分母分离"的选择既让合并逻辑干净，又避开了 GPU 上昂贵的除法指令——它就是 8.3 节"延迟归一化"的数学根源（FlashAttention-1 早期形式在循环内维护归一化的 O，FA-2 改为维护 o~，是其主要提速点之一）。

### 6.5 从一维到二维：m、l、o~ 升级为"逐行"的向量

到目前为止的推导都在一维向量（S 的一行）上进行。推广到 Attention 的真实形态只需一步：softmax 是**按行独立**的（1.4 节），所以**每一行都有自己的一套 (m, l, o~)**——把 Q 按行切成块（每块 Br 行）后，块的统计量从标量升级为向量：

```
一维（单行）:   m, l 是标量,       o~ 是 d 维向量
二维（Br 行）:  m, l 是 Br 维向量,  o~ 是 Br×d 矩阵      （每行一套，互不相干）
```

对应地，K/V 沿序列切成列块（每块 Bc 行）。一次处理的分数是 Br×Bc 的小矩阵 S_blk = Q_blk·K_blkᵀ，更新流程**逐行并行**：

```
m_blk = rowmax(S_blk)                        Br 维：每行在本块内的最大值
m_new = max(m, m_blk)                        逐元素取大
r     = exp(m − m_new)                       Br 维补偿系数：每行一个
l     = l ⊙ r + rowsum(exp(S_blk − m_new))   ⊙ = 逐元素乘
o~    = o~ ⊙ r + exp(S_blk − m_new) · V_blk  r 沿行广播到 o~ 的每一行
m     = m_new
```

与一维版逐条对应，只是所有标量运算换成了"沿行的向量运算 + 广播"。两个容易误解的要点：

- **m 不是全矩阵的最大值**。它是"每行各自的最大值"拼成的列向量——行与行的归一化互不影响，这是 softmax 按行定义的直接结果；
- **行独立性正是"Q 按行分块"合理性的来源**：某一行的 query 看完所有 K/V 块后，这一行的输出就完整了，与其他行何时算完毫无关系。N×N 分数矩阵自始至终不需要以任何形式完整存在。

至此，FlashAttention 的全部数学已经就位；剩下的是把这套"逐行向量递推"映射到 GPU 的线程组织上——第 7 章的实现中，Br 维的 m/l 恰好按"每线程一行"放进各线程的寄存器。
...
本章已经把全部数学准备好了：6.2 的逐元素递推、6.3 的分块合并、6.4 的分子分母分离、6.5 的逐行向量化。剩下的是纯粹的工程问题——把"每次处理 K/V 的一个 tile，块内小矩阵乘（用满 GPU 算力），块间一次修正"落到 CUDA 的线程组织、共享内存与寄存器上。这就是 FlashAttention。
...
把 V2 的递推按 6.3/6.5 节的分块合并形式组织起来，并让所有中间量驻留片上：
...
正确性完全由 6.2~6.5 节的递推与合并公式保证——FlashAttention 是**精确算法**，不是近似。

### 7.3 教学版 CUDA 实现

下面给出一个以可读性为先的实现：每个 Block 处理一个（batch×head 的）Q 行块，`blockDim.x = Br`，**每个线程负责块内一行**——q、o~、m、l 全部驻留该线程的寄存器（正是 6.5 节"每行一套统计量"到线程组织的直接映射），K/V tile 由全 Block 协作装入共享内存：
...
除并行度外，还可以从**状态的存放位置**算一笔账（设 Q 切成 Tr 块、K/V 切成 Tc 块）。FA-1 的外层在 KV 上，意味着"累加中的 O"是所有 Q 块共享的进度——片上装不下全部 O，只能放显存：

```
FA-1（外层 KV）: 处理每个 KV 块时，都要把各 Q 块的 (o~, m, l) 从 HBM 读入、
                更新、再写回 —— O 级数据要过 HBM 约 2×Tc 遍
                （N=4096、Bc=64 时 Tc=64：o~ 被反复搬运上百遍！）
FA-2（外层 Q） : (o~, m, l) 是当前 Q 块的私有状态，全程驻留寄存器/共享内存，
                内层循环一次不碰 HBM，每个 O 块只在收尾时写出 1 次
```
...
- **归一化推迟到最后**：内层循环只维护未归一化的 o~（V2 的 6.4 节本就如此），不在每块后除以当前 l；
- **修正因子合并**：rescale 乘法尽量批量地作用于 o~，减少每块的标量乘次数。

为什么"每块先归一化"（直觉上更符合 softmax 输出概率的定义、也是 FA-1 早期形式的做法）反而是坏主意？6.4 节的合并推导已给出数学层面的答案（合并时要乘回 d₁ 恢复分子，归一化被原样抵消），这里补上硬件层面的两条：

1. **合并变绕**：块内归一化后的 O₁ 在合并时必须先乘 d₁ 还原成分子再参与累加——"除了又乘回来"纯属浪费，还让代码逻辑复杂；
2. **除法是慢指令**：GPU 上浮点除法/倒数的吞吐远低于乘加（FMA）。放在内层循环意味着每个 KV 块都付一次代价，Tc 个块就是 Tc 次；推迟到循环外，全程只付一次。

于是内层循环中**分子 o~ 与分母 l "兵分两路"**：各自用同一补偿系数独立累加，l 全程不参与 o~ 的更新，唯一的一次相除发生在所有块处理完之后（7.3 教学版代码中的 `inv_l` 正是这一步）。
...
归并公式正是 Online Softmax 的"两两合并"形式（6.3 节的分块合并公式原封不动地再次登场，可体会该恒等式的普适性）：
...
写 CUDA 之前，可以先用纯 PyTorch 把分块递推"演"一遍——既验证第 6 章的数学（在线合并是否精确），又能直观感受 8.1 节循环顺序的影响。下面的代码同时实现 FA-1 风格（外层 KV）与 FA-2 风格（外层 Q），核心数学完全一致，只交换了循环内外与状态的存放位置：
...
在一块典型 GPU 上的实测（N=4096，10 次平均，含 `torch.cuda.synchronize()`）：

| 实现 | 时延 | 说明 |
|------|------|------|
| FA-1 风格（外层 KV） | ~680 ms | 每个内层步都对全局 O/m/l 切片读改写 |
| FA-2 风格（外层 Q） | ~404 ms（约 1.7×） | 状态是局部变量，每个 O 块只写一次 |
...
2. **加速比"只有"约 1.7×，远小于真实 CUDA 实现中 FA-2 对 FA-1 的差距**。原因是 PyTorch 模拟有个突破不了的天花板：`o = o * corr + P @ V_j` 中每个算子的结果张量都落在 HBM 上——"状态驻留片上"只是逻辑上的，物理上做不到；Python 循环还叠加了每次迭代的 kernel 启动开销。真正的 CUDA 实现中 o~/m/l 锁在寄存器里、内层循环全程不碰 HBM，收益才完整兑现。把 N 加大到 8192 以上再跑，会看到加速比继续上升——FA-1 的状态搬运总量随块数按 Tr×Tc（平方级）增长，FA-2 只随 Tr（线性）增长。
...
| 动手实践 | 先跑 10.3 的 PyTorch 模拟找感觉，再实现 7.3 教学版，用 10.2 的方法对拍，再用 ncu 看流量 |
...
| 分块合并（两两归并） | 各块局部 (m, d) 乘补偿系数后相加；可结合、可树状并行归并 | 第 6、9 章 |
| 补偿系数（换基准） | exp(m_old − m_new)，把以旧 max 为基准的部分和换到新基准 | 第 6 章 |
| 未归一化输出 o~ | Σ exp(sⱼ−m)·vⱼ，与 l 同规律递推，最后一除即得 O | 第 6~7 章 |
| 分子分母分离（延迟归一化） | 内层只累加分子 o~ 与分母 l，除法推迟到循环外一次完成 | 第 6、8 章 |
