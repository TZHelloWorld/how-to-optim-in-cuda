...
- [5.2 全局内存访问深入：合并法则、访问模式与硬件机制](#52-全局内存访问深入合并法则访问模式与硬件机制)
  ...
  - 进阶选读（硬件机制）：5.2.7 内存段与对齐｜5.2.8 内存事务与事务粒度｜5.2.9 缓存行与扇区（sector）｜5.2.10 为何行 128B、粒度 32B｜5.2.11 一次访存的完整旅程｜5.2.12 概念体系总览
...
> [!NOTE]
> 本地内存有一个容易被忽略的官方设计：它的排布方式是**连续的 32 位字由连续的线程 ID 访问**——也就是说，只要 warp 内所有线程访问"相对位置相同"的本地变量（比如各自 `tmp[3]`、结构体的同一个成员），这些访问在物理上**天然是完全合并的**（官方原文：*"Accesses are therefore fully coalesced as long as all threads in a warp access the same relative address"*）。所以本地内存慢在"延迟与全局内存同级"，而不是"访问模式差"——它相当于一个由硬件替你做好 SoA 交错布局的每线程私有区。
...
使用 `__constant__` 声明，容量通常为 64 KB，设备端只读。当 warp 内所有线程读**同一个地址**时经常量缓存一次广播完成，效率极高；若各线程读不同地址则串行化。官方把这笔账算得很精确：**成本随 warp 内不同地址的数量线性增长**（"the cost scales linearly with the number of unique addresses read by all threads within a warp"），而全 warp 读同一位置且缓存命中时，**常量内存可以和读寄存器一样快**。因此它适合存放系数、查找表等所有线程**同步读同一项**的只读参数——"人人读一样的"是它的主场，"人人读不一样的"是它的坟场。
...
注意参数必须是之前 `cudaMalloc` 类函数分配的空间——输入非法指针参数会返回错误（老版本文档写作 `cudaErrorInvalidDevicePointer`，该错误码已弃用，现行 Runtime API 返回 `cudaErrorInvalidValue`），重复释放同一块空间也会报错；传入 `nullptr` 则是合法的空操作。
...
- GPU 内的所有 SM 共享一个更大的 **L2 缓存**。**所有**全局内存访问都会经过 L2。按官方（Nsight Compute）的精确定义：**L1 和 L2 的缓存行（cache line）都是 128 字节，每行分为 4 个 32 字节的扇区（sector）**，而 sector 才是数据传输与访问的最小单位——这就是为什么在现代 GPU（Compute Capability 6.0+）上，全局内存访问的基本事务粒度是 32 字节。
- 每个 SM 还有独立的**常量缓存**。

为什么缓存行是 128 字节、最小访问粒度却是 32 字节？两级缓存又如何协同工作？这是理解全局内存访问效率的关键，[5.2 节](#52-全局内存访问深入合并法则访问模式与硬件机制)将展开完整分析。
...
- 每 SM 一个 L1，全芯片共享一个 L2，所有全局访存必经 L2；**L1/L2 缓存行均为 128 B，行内分 4 个 32 B 扇区（sector），sector 是最小访问粒度**。
...
> 全部分只有一条主线：**warp 是访存的基本单位——warp 内 32 个线程的内存请求会被硬件合并（coalesce）成尽可能少的内存事务**。事务越少、每个事务中有效数据越多，带宽利用率越高。（内容依据 CUDA C++ Programming Guide "Device Memory Accesses"、Best Practices Guide "Coalesced Access to Global Memory" 与 PTX ISA "Cache Operators" 各节——官方文档的章节编号随版本变动，本章一律用小节名引用；正文尽量以中文结论呈现，不再逐段摘抄原文。）

在钻进细节之前，先用一个贯穿本部分的类比把直觉立起来。把 DRAM 想象成一座**只按"整箱"发货的仓库**：

- 货架按固定尺寸分箱（32/64/128 字节）——这是**内存段**；
- 每次出库必须搬走完整的一箱，哪怕你只要其中一件——这是**内存事务**的"全取或不取"；
- 车间门口有两级暂存区：每个车间私有的小货架（L1）和全园区共享的中央仓（L2），两者的货架格子都是 128 字节一格（**缓存行**），但格子内按 32 字节的**小箱（sector）**记账和进货；
...
情况4：极端跨步（stride=32）
  ...
  → 32 箱，128/1024 字节有效，利用率 12.5%（若按 128 字节缓存行计：
    32 行 × 128 B = 4096 B，利用率仅 3.1%；实测带宽通常也在个位数百分比）
```
...
| 跨步 stride=32 | 32 | 12.5%（实测可低至 ~3%） | 每人独占一箱、各拿一件，还占满 32 个缓存行，几乎全是浪费 |
...
有了黄金法则，本节把各种典型访问模式逐一画出来，按"经 L1 加载、经 L2 加载、写入"三种路径分别分析带宽利用率。先交代一个马上会在小节标题里出现的编译开关：`-Xptxas -dlcm=ca`（默认）表示加载**经 L1+L2 缓存**、以 128 字节（L1 缓存行）为粒度；`-Xptxas -dlcm=cg` 表示**绕过 L1、只经 L2**、以 32 字节为粒度（背后的机制见进阶篇 5.2.10 节）。

> [!NOTE]
> 一个重要的时代限定：这套"经 L1 就按 128 字节计事务"的分析出自经典教材，对应 **Fermi/Kepler 时代**的硬件语义。**CC 6.0+ 的现代 GPU 上，无论加载是否经过 L1，数据访问粒度都统一是 32 字节 sector**（5.2.8.2 节的官方结论）——也就是说现代硬件在"128 字节粒度 vs 32 字节粒度"的对比中天然站在细粒度一侧。本节仍完整保留两种粒度的对比图解，原因有二：① 它把"粒度大小如何影响利用率"讲得最直观，是理解 5.2.10 节 sectored cache 设计动机的最好铺垫；② 各种访问模式的**相对**优劣结论（对齐优于非对齐、连续优于跨步）在两种粒度下完全一致。

再明确两个基本概念：

- **对齐内存访问（Aligned Memory Access）**：当内存事务的第一个地址是访问粒度（L2 的 32 字节 sector 或 L1 的 128 字节行）的整数倍时就是对齐内存访问。运行非对齐的加载会造成带宽浪费。
...
- **缓存的管理粒度**：L1/L2 的缓存行均为 128 字节、按 32 字节 sector 填充数据（5.1.8 节）；本节图解中"L1 路径按 128 字节、L2 路径按 32 字节"分析的正是这两种粒度。对线程而言，**读取全局内存一定会经过 L1/L2 缓存中的至少一个进行加载**；
- **内存控制器传输粒度**：DRAM 控制器以固定大小的内存事务为基本传输单位，该单位与 sector/缓存行或其倍数对齐。
...
5. **最坏的情况：目标数据分散在内存的各个角落**：需要 N 个内存段。此时细粒度的优势最明显——L1 模式要加载 $N \times 128$ 字节，而 L2 模式只需 $N \times 32$ 字节，无用数据少拉了 3/4。这里假设 N 不随粒度变化；实际情况中使用大粒度缓存行时 N 有可能会减小（多个分散地址恰好落进同一个 128 字节行）：
...
> [!NOTE]
> "写不经过 L1"是 Fermi/Kepler 时代教材的经典表述。Volta（CC 7.0）起 L1 支持缓存写入，但采用**写直达（write-through）**策略——写入的数据会进 L1，同时**必须**继续写到 L2，所以对写操作而言 L1 起不到"吸收流量"的作用，性能分析上"写的成本以 L2/显存为准、粒度按 32 字节 sector 计"这个结论在新老架构上都成立。这也是 5.2.5.3 节"优先保写合并"策略在现代架构依然有效的原因：读侧的交叉能被 L1 命中缓解，写侧不能。
...
`main.cu` 复现了 Best Practices Guide "A Sequential but Misaligned Access Pattern"（offset copy）与 "Strided Accesses"（strided access）两小节的经典实验，实测两类非理想访问模式对有效带宽的影响：
...
> **进阶选读分界线**：到这里，"怎么写出高效访存代码"的实用部分已经完整。下面六节（5.2.7~5.2.12）把前面当作公理的三条硬件事实（32 字节、对齐、全取或不取）追到硬件根源——依次回答"DRAM 为什么按段划分""一次访问到底产生几个事务""缓存行与扇区（sector）是什么""为什么行是 128 字节、粒度却是 32 字节""一次访存在两级缓存中怎么走"。第一遍学习可以直接跳到 [5.2.13 小结](#5213-小结全局内存)；当你需要用 Nsight Compute 分析事务数、想真正读懂 profiler 指标时，再回来精读。
...
GPU 的设备内存（device memory，即 DRAM / HBM）不是逐字节随意访问的，而是通过固定大小的 **内存段（segment）** 来访问。每次内存事务操作的就是一个完整的段。官方的规定（Programming Guide "Device Memory Accesses" 一节）翻译过来就一句话：**全局内存只能通过 32、64 或 128 字节的内存事务读写，且事务必须"自然对齐"——段的首地址必须是段大小的整数倍**。

**内存段就是硬件将 DRAM 地址空间按固定大小切分后的每一个地址区块**。全局内存的段大小有三种粒度：

| 段大小 | 对齐要求 | 说明 |
|--------|---------|------|
| 32 字节 | 首地址必须是 32 字节的整数倍 | 最小事务粒度（现代 GPU 的基础事务单元 / sector 大小） |
| 64 字节 | 首地址必须是 64 字节的整数倍 | 中间粒度 |
| 128 字节 | 首地址必须是 128 字节的整数倍 | 最大事务粒度 / 缓存行大小（L1、L2 均为 128 B） |
...
官方结论（Best Practices Guide "Coalesced Access to Global Memory" 一节）：**warp 连续但未对齐到 32 字节段的访问，会请求 5 个 32 字节段**（对齐时只需 4 个）；段是"全取或不取"的最小单位，多请求的段里混着无人需要的字节，就是纯粹的带宽浪费。极端情况下浪费可以非常惊人——若每个线程的 4 字节访问都各自触发一个 32 字节事务，吞吐量直接除以 8（Programming Guide "Device Memory Accesses" 一节）。
...
Best Practices Guide "A Sequential but Misaligned Access Pattern" 一节在 Tesla V100 上的实测：理论损失为 4/5 = 80% 吞吐，实测约为 9/10 = 90%——因为 L1 的 128 字节大缓存行使得**相邻 warp 可以复用彼此"多取"的数据**，弥补了部分性能损失（这正是 5.2.2 节利用率表中"实测约 90%"的出处）。
...
官方明确警告（Programming Guide "Device Memory Accesses" 一节）：对于 `double`（8 字节）、`float4` / `double2`（16 字节）等宽类型，**读取未自然对齐的 8/16 字节字会产生错误结果**（错位若干个字）——不只是性能问题，必须严格保证对齐：
...
L2       ↔ L1       32 字节       32 字节段
L1 缓存行填充        128 字节      128 字节段（一行 4 个 sector 全填充时）
```
...
官方结论（Best Practices Guide "Coalesced Access to Global Memory" 一节）：CC 5.x 时代 L1 默认关闭、开启后按 128 字节段计事务；**CC 6.0 起 L1 默认开启，且无论是否经过 L1，数据访问单元统一为 32 字节**：
...
Programming Guide（"Device Memory Accesses" 一节）给出了决定事务数的两个因素：
...
#### 5.2.9.2 缓存行与扇区（sector）：官方的精确定义

许多教材习惯说"L1 行 128 字节、L2 行 32 字节"——这个**行为模型**能算对事务数，但官方术语更精确。Nsight Compute 官方文档的定义是：

> **Sector**: *"Aligned 32 byte-chunk of memory in a cache line or device memory. An L1 or L2 cache line is four sectors, i.e. 128 bytes."*（sector 是缓存行或设备内存中对齐的 32 字节块；**L1 和 L2 的缓存行都是 4 个 sector，即 128 字节**。）

也就是说，现代 GPU 的 L1/L2 都是**扇区化缓存（sectored cache）**，有两层粒度：

| 概念 | 大小 | 作用 |
|------|------|------|
| **缓存行（cache line）** | 128 字节（L1 与 L2 相同） | 地址标签（tag）管理的单位——一个槽位对应一段 128 字节对齐的地址范围 |
| **扇区（sector）** | 32 字节（每行 4 个） | **数据填充与传输的最小单位**——miss 时只取缺的 sector，不必填满整行；官方明确 "the minimum access size in L1/L2 is one sector" |

这个两层设计正是"32 字节事务粒度"与"128 字节缓存行"能同时成立的原因：**tag 按行管理省开销，数据按 sector 搬运省带宽**。命中判定也据此分两级：tag 命中且所需 sector 在行内才算命中；tag 命中但 sector 缺失（tag-hit-data-miss）仍需去下级取那个 sector。

> [!TIP]
> 这个概念直接对应 profiler 指标：Nsight Compute 大量使用 **sectors per request（每次请求的扇区数）**衡量合并程度。官方给出的最优参考值——**32 个活跃线程、每线程访问 4 字节时，理想值为 4**（每 8 个连续线程共享一个 sector）；8 字节为 8、16 字节为 16。实测值越高于理想值，说明访问越分散（最坏 32：每线程独占一个 sector）。5.2.2 节"数箱子"的箱数，在 profiler 里的名字就是 sectors。

#### 5.2.9.3 缓存行、扇区与事务粒度的关系

三者是描述同一物理行为的不同视角：

```text
事务粒度（Transaction Granularity）= 32 字节
  → 内存总线视角：一次物理传输搬运多少字节

扇区（Sector）= 32 字节
  → 缓存存储视角：缓存行内数据填充/命中判定的最小块

缓存行（Cache Line）= 128 字节（L1 与 L2 相同）
  → 缓存管理视角：一个 tag 槽位覆盖的地址范围 = 4 个 sector

对应关系（CC 6.0+）：
  事务粒度（32 字节） ≡ sector（32 字节）
  缓存行（128 字节）  = 4 × sector
```

另有一个相近概念——**数据访问单元（Data Access Unit）**：从 warp 视角描述的最小请求单位，在 CC 6.0+ 中与事务粒度/sector 数值相同（均为 32 字节），但描述侧重不同：事务粒度强调硬件总线的"物理传输"，数据访问单元强调 warp/软件"一次请求触发的最小数据量"。

### 5.2.10 深入硬件（四）：为什么行是 128 字节、粒度却是 32 字节？

上一节给出了官方的两层结构：缓存行 128 字节、扇区 32 字节。为什么不干脆"行多大就搬多大"？这个"大行小扇区"的组合不是偶然，而是两种粒度各取所长的刻意设计。
...
#### 5.2.10.2 大行、小扇区：两种粒度的分工

| 设计维度 | 缓存行 = 128 字节（管理粒度大） | 扇区 = 32 字节（传输粒度小） |
|---------|------------------------------|---------------------------|
| **省什么** | 省 **tag 开销**：一个地址标签管 128 字节；若按 32 字节配 tag，标签存储和查找电路要翻 4 倍 | 省**带宽**：miss 时只取真正缺的 sector，不用为一个 4 字节的请求搬满 128 字节 |
| **利好谁** | **规整的顺序访问**：warp 连续读时 4 个 sector 同行填充，相邻 warp 大概率命中同一行（空间局部性红利） | **分散/跨步访问**：每个落点只花 32 字节的代价，避免缓存被无人使用的数据污染 |
| **代价** | 行内可能有从未被用到的 sector 占着位置 | 分散访问时 tag 命中、sector 缺失（tag-hit-data-miss）需多次回下级补数据 |

**核心设计逻辑**：如果只有 128 字节一种粒度，分散访问会为每个落点搬 128 字节、浪费 4 倍带宽（这正是 CC 5.x 时代"开 L1 按 128 字节计事务"对分散访问不友好的原因，5.2.3.2 节的图解对比过）；如果只有 32 字节一种粒度，tag 管理开销翻 4 倍、顺序访问也占不到预取的便宜。**扇区化缓存（sectored cache）= 用 128 字节行做便宜的管理 + 用 32 字节扇区做精确的搬运**，规整与分散两头兼顾——这正是 5.2.3.2 节末尾那条 TIP（"CC 6.0+ 统一为 32 字节数据访问单元，兼得两者优点"）背后的硬件形态。

#### 5.2.10.3 PTX Cache Operators 对 L1/L2 的独立控制

在指令层面，PTX ISA（"Cache Operators" 一节）允许对两级缓存分别控制，最常用的两个 load 缓存操作符是：
...
第 3 步：查询 L2 Cache（sector = 32 字节）
─────────────────────────────────────────────────────────────────
L2 以 32 字节 sector 为粒度，对每个 32 字节请求独立查询：
  ...
  [96, 128) → L2 命中？ ┘  4 × 32 字节 = 128 字节 传到 L1
                         ...
                         ┬─ 否 → 向 DRAM 发起 32 字节事务
                         │       DRAM 返回 32 字节数据
                         │       → 填入 L2（缓存行内对应的 32 字节 sector）
                         └       → 聚合后填入 L1（128 字节缓存行）
                                  → 返回数据（约 600~800 个周期）
```

> [!NOTE]
> 本例中 warp 恰好用到全部 4 个 sector，所以"填满一整行"；若 warp 只请求其中 1 个 sector，扇区化缓存**只会填充缺失的那个 sector**，行内其余 sector 位置留空（这正是 5.2.9.2 节 tag 命中但 sector 缺失的来源）。

#### 5.2.11.2 L1 的 128 字节缓存行如何由 32 字节 sector "拼凑"而来

```text
从 L2（或 DRAM）分 4 次收到的 32 字节 sector：
...
L1 将 4 个连续 32 字节 sector 填入同一个 128 字节缓存行的 4 个扇区槽位。
这就是"行按 128 字节管理、数据按 32 字节搬运"协同工作的核心机制。
```
...
关键：相邻 warp 的缓存复用机会
  ...
  → 缓存行B 已在 L1 中 → L1 命中！
  → 这正是 Best Practices Guide 非对齐访问实验一节所说：
    "adjacent warps reuse the cache lines their neighbors fetched"
    （相邻 warp 复用彼此预取的缓存行）
```

正因为 L1 缓存行覆盖 128 字节（4 个 sector），相邻 warp 的访问地址往往都落在同一个 128 字节缓存行内，从而产生大量缓存复用，弥补了非对齐带来的部分性能损失。

#### 5.2.11.4 跨步访问（Strided Access）的缓存命中影响

Best Practices Guide（"Strided Accesses" 一节）：
...
```text
stride=1（连续访问）：4 个 32 字节 sector → 128 字节 → 1 个 L1 缓存行，100% 利用率
stride=2（间隔访问）：8 个 32 字节 sector → 256 字节 → 跨 2 个 L1 缓存行，50% 利用率
stride=32（极端跨步）：32 个 32 字节 sector → 1024 字节 → 各落在 32 个不同的缓存行，
                       sector 利用率 12.5%；按行计 32×128B=4096B，仅 3.1%，实测带宽同量级
```
...
| **扇区（Sector）** | 缓存行内数据填充/命中判定的最小块（官方定义的 32 字节对齐块） | **32 字节** | 缓存数据传输视角 |
| **缓存行（Cache Line）** | 一个地址标签（tag）覆盖的范围 = 4 个 sector，**L1 与 L2 相同** | **128 字节** | 缓存管理视角 |
...
warp 发出 load 指令（访问 float arr[tid]）
  ...
  ┌─────────────────────────────────────┐
  │            L2 Cache                 │  ← 全芯片共享，片上
  │   缓存行 128 字节 = 4 × 32B sector   │    每个 32B sector 请求独立查询
  │       (~200 周期延迟)               │    4 次 sector 命中 → 返回给 L1 填缓存行
  └──────────────┬──────────────────────┘
...
- CC 6.0+ 统一以 **32 字节**为事务粒度/数据访问单元，官方术语是**扇区（sector）**；L1/L2 缓存行均为 128 字节 = 4 个 sector（tag 按行管理、数据按 sector 搬运）；profiler 里对应 **sectors per request** 指标（4 字节访问的理想值为 4）；
...
> [!NOTE]
> 官方对"同一个字"给出了两条精确规则，判冲突时经常用到：① **只要多个线程访问的地址落在同一个 32 位字内，就不产生 bank 冲突**——哪怕这些地址确实属于同一个 bank（读时一个线程取数后广播/多播给其余线程）；② **多个线程写同一位置时，只有一个线程会写成功，而且是哪一个是未定义的**（"only one thread succeeds in the write; which thread succeeds is undefined"）——共享内存不会帮你做写仲裁，需要确定结果就用原子操作（第 7 章）。
...
- **8 字节（64-bit）bank 宽度**：仅 Kepler 架构（当年通过 `cudaDeviceSetSharedMemConfig` 配置；该 API 随 Kepler 退役已被弃用，在现代架构上调用无效果，新代码无需理会）。
...
- **小于 4 字节的类型（`char` / `short`）**：多个相邻元素落在同一个 bank 的同一个 4 字节字内。如 `char shared[64]` 中，线程 0~3 访问 `shared[0..3]` 时四个字节属于同一个 word——按 5.3.3 节 NOTE 的官方规则①，同字访问不算冲突（读走广播/多播）；但写入同一 word 的不同字节时可能因竞争而串行化；
- **8 字节类型（`double`）**：warp 一次请求 32 × 8 = 256 字节，超出共享内存每周期 128 字节（32 bank × 4 字节）的供数能力，硬件必然拆成 **2 个访问波（wavefront）**分两拍完成。注意：**连续访问 `double shared[32]` 时这 2 拍各自无 bank 冲突**——"拍数翻倍"是位宽的必然开销，不是冲突，Nsight 的 bank conflict 指标也不会计入；只有当访问模式让某一拍内两个线程落到同一 bank 的**不同** word 时才是真冲突（例如跨步访问 double 数组）；
- **16 字节类型（`float4` 等）**：同理拆成 **4 个 wavefront**，连续访问时各拍无冲突。宽类型的代价体现在"共享内存吞吐被位宽稀释"，设计 tile 布局时要把这笔账算进去，而不是当作冲突去消除。

> [!NOTE]
> "wavefront（访问波）"是官方 Nsight Compute 的术语：L1TEX 流水线每周期最多通过一个 wavefront，一次请求需要的 wavefront 数 = 理想拍数 + 冲突造成的额外拍数。**profiler 里的 bank conflict 指标统计的是"超出理想拍数的部分"**——所以连续 double 访问显示 0 冲突、但吞吐减半，两件事并不矛盾。
...
工具齐备，回头解决 5.2.5 节遗留的矩阵转置问题：朴素版本中对输出矩阵的写操作无法合并。使用共享内存 tile 做中转 + padding 消除 bank 冲突，读写两侧都能达到合并访问（对应 [`code/transpose_smem.cu`](code/transpose_smem.cu) 中的版本 3，该文件还包含朴素版与无 padding 版共三个版本供对比）：
...
- 步长规律：`gcd(32, s) = 1`（奇数步长）无冲突；分析宽类型时按实际触碰的 4 字节字计算——**double/float4 连续访问的 2/4 拍（wavefront）是位宽的必然开销，不算 bank 冲突**；
...
| 主题 | 官方文档 |
|------|---------|
| CUDA C++ 编程指南：设备内存访问（合并、事务、对齐、本地/常量内存——5.2 部分主要出处） | https://docs.nvidia.com/cuda/cuda-c-programming-guide/index.html#device-memory-accesses |
| CUDA C++ 最佳实践指南：合并访问（offset/stride 实验数据的出处） | https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/index.html#coalesced-access-to-global-memory |
| CUDA C++ 最佳实践指南：共享内存与锁页内存 | https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/index.html#shared-memory |
| Nsight Compute Metrics Guide：硬件模型与内存表（sector/cache line 官方定义、sectors per request 指标） | https://docs.nvidia.com/nsight-compute/ProfilingGuide/index.html#metrics-guide |
| CUDA C++ 编程指南：计算能力各代共享内存/L1 规格（5.3.1 容量表出处） | https://docs.nvidia.com/cuda/cuda-c-programming-guide/index.html#compute-capabilities |
| PTX ISA：Cache Operators（`.ca`/`.cg` 等缓存操作符） | https://docs.nvidia.com/cuda/parallel-thread-execution/index.html#cache-operators |
