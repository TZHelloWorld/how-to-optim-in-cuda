...
给定字符串 `s`，求其中不含重复字符的最长子串的长度。子串必须占据原字符串中一段连续位置，不允许跳过字符；返回长度，不需要返回子串本身。

**输入与约束**：s 可以为空，也可能包含字母、数字、符号和空格。不同位置的相同字符算重复，空字符串结果为 0。

```text
输入：s="abcabcbb"
输出：3
解释："abc" 是长度为 3 的无重复子串。

输入：s="pwwkew"
输出：3
解释："wke" 合法；"pwke" 虽无重复，但不连续，不能作为答案。
```

### 从题意到算法

枚举所有子串再去重会重复检查。让右端每次增加一个字符，左端只在发生重复时前进，直到旧的重复字符被移出。此时窗口既合法，又是当前右端下能保留的最长窗口；记录所有右端的最大长度即可。一次删除可能不够，所以收缩要使用 while。
...
给定非空字符串 `s` 和 `p`，找出 s 中所有与 p 互为字母异位词的子串，返回这些子串在 s 中的起始下标。异位词必须包含完全相同的字符及次数，只允许顺序不同。

**输入与约束**：两个字符串均只含小写英文字母；结果可以重叠，返回顺序不限。若 p 比 s 长或没有匹配，返回空列表。

```text
输入：s="cbaebabacd", p="abc"
输出：[0,6]
解释：s[0:3]="cba"、s[6:9]="bac" 都是 abc 的异位词。
...
匹配窗口长度必须恰好等于 `len(p)`，无需枚举其他长度。目标和窗口各保存 26 个字母计数：右端加入一个字符，如果超长就移出最左字符，长度达到要求时比较计数。每次只增减两个位置，比对每个窗口单独排序更直接。
